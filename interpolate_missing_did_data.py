...
def reindex_and_fill_in_dataframe(df, DAYS_BWD, DAYS_FWD, idx_from_product=True):
    ...
    exists are filled in with 0 for the follower gain rate.

    Inputs:
      df: a polars dataframe with columns unit_id, time_period, ever_treated, ts, and gain_rate.
    
    Outputs:
      df with the same columns but now using a MultiIndex for unique combinations of
      (unit_id, ever_treated, ts)
    """
...
def interpolate_arrays(row):
    ...
    Input:
        row: dataframe; either the subset of gr where ever_treated is True or False. 
          Must have columns unit_id, time_period, ever_treated, ts, and gain_rate
...
def complete_interpolation_for_unit(gr):
    ...
    Input:
        gr: dataframe; one group from a pandas GroupBy. 
          must have columns unit_id, time_period, ever_treated, ts, and gain_rate
...
def get_time_period_by_unit(df, group_with_repost_creation=False):
    ...
    This will become the time_period column in the final dataframe

    Inputs:
      df: polars dataframe with columns unit_id, time_period, ever_treated, ts, and gain_rate.

...
* `interpolate_missing_did_data.py`: `parse_reposts_and_extract_follow_timings.py` and `get_non_treated_units_per_repost.py` do not produce rows for combinations of {day, reposted account, follower/non-follower} where no follows occurred. In order to run the actual differences-in-differences analysis or visualize the per-day per-capita follow rates, we need to add in those missing rows and assign a follower count of $0$. This is what this script does. We run it as `python3 interpolate_missing_did_data.py $HANDLE $DAYS_FWD $DAYS_BWD $ACCT_TYPE`. `HANDLE` is the attention broker's Bluesky handle; `DAYS_FWD` and `DAYS_BWD` refer to the number of days of data we collect after & before the repost, respectively; and `ACCT_TYPE` is either `did`, denoting the reposted account data from `parse_reposts_and_extract_follow_timings.py`, or `control`, denoting the control data from `get_non_treated_units_per_repost.py`. 
* `parse_reposts_and_extract_follow_timings.py`: For all accounts reposted by an attention broker, makes CSVs with the following columns:
...
* `get_non_treated_units_per_repost.py`: For all accounts A reposted by an attention broker, randomly selects 3 accounts followed by the attention broker that were *not* reposted by the attention broker prior to the time A was reposted + 14 days. These are our control accounts. For each account, we add same information as in `parse_reposts_and_extract_follow_timings.py`:
    * `gain_rate`: number of followers accumulated by the control account
    * `ever_treated`: Boolean indicating whether the followers accumulated are followers of the attention broker or not
    * `unit_id`: Identifies the control account
    * `time_period`: Number of days elapsed since the earliest repost in the dataset (keeps track of absolute time)
    * `ts`: Days relative to the repost event (ranges from -1 * DAYS_BWD to DAYS_FWD).
Note that `unit_id` continues the count of units where `parse_reposts_and_extract_follow_timings.py` finished. We run this script as `python3 get_non_treated_units_per_repost.py HANDLES_IN.txt $DAYS_FWD $DAYS_BWD`, where `HANDLES_IN.txt` is a .txt file with an attention broker's Bluesky handle on each line. `DAYS_FWD` refers to the number of days after the repost for which we're collecting data, and `DAYS_BWD` refers to the number of days before the repost for which we're collecting data. Right now I'm using 14 for both.
* `utils.py`: Contains utility functions for data parsing.

### Other Code Files
* `plots.ipynb` contains functionality for plotting overall trends in per-capita per-day follow rates for control and reposted accounts for followers and non-followers (a total of 4 curves are plotted). It also produces a dataframe for use in `diff_in_diff.R`. 
* `diff_in_diff.R` conducts an event study using Poisson regression. It plots the event study results, and it has functionality to output regression results in raw LaTeX code. 
...
* `plots/event_studies_poisson` contains event study plots, which show the causal effect of the repost on following rates over time.
...
* `interpolated_did_csvs/*.csv` contains the output of `interpolate_missing_did_data.py` for each attention broker we studied in this analysis (reposted accounts).
* `interpolated_control_csvs/*.csv` contains the output of `interpolate_missing_did_data.py` for each attention broker we studied in this analysis (control accounts).
* `processed_control_csvs/*.csv` contains the output of `plots.ipynb` that is used for the diff-in-diff analysis done in R (for control accounts).
* `processed_did_csvs/*.csv` contains the output of `plots.ipynb` that is used for the diff-in-diff analysis done in R (for reposted accounts).
